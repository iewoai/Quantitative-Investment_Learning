...
def reallocate(context, bar_dict):
	...
	f_needs = get_needs(context, bar_dict, date)
	log.info(f_needs)

# 获得因子函数
def get_needs(context, bar_dict, date):
	...
	q = query(
		...
		factor.symbol.in_(context.iwencai_securities),
	...
	# 第二步暂时去除
	# 可能出现合格因子数目很少的情况
	# 拟定：当第一步的因子大于最大因子数目进行第二步，小于则直接进行第三步
	if len(eff_need) > context.need_max:
		# 2.单因子单股票回归性检验
		...
		for stock in context.iwencai_securities:
			...
			# get_fundamentals获取财务数，get_factors获取因子数据(暂时将缺失值补充为0)
			df = get_factors(q).fillna(0)

			# 去除所有值都缺失的列
			df = df.ix[:,~((df==0).all())]

			prices = get_price(stock, last_year_date, date, '1d', ['close'], skip_paused = False, fq = 'pre',is_panel = 0)
			stock_close = prices['close'].values
			stock_rate = np.diff(stock_close) / stock_close[:-1]

			df = df.iloc[:-1, :]
			df['rate'] = stock_rate

			a = {}
			# 去极值-标准化（前两列名为 factor_date，factor_symbol，最后一列为rate）
			for index, row in list(df.iteritems())[2:-1]:
				df[index] = standardize(filter_extreme_3sigma(row))

				a[index] = [t_value(df[index], df['rate'])]

			
			t_data = t_data.append(pd.DataFrame(a), ignore_index=True)

		for index, row in list(t_data.iteritems()):
			log.info(index)
			a = list(map(abs, list(t_data[index])))
			log.info(np.mean(np.array(a))
	

		
		# 获取所有股票每个交易日收盘价
		prices = get_price(context.iwencai_securities, last_year_date, date, '1d', ['close'], skip_paused = False, fq = 'pre',is_panel = 0)

		for stock in context.iwencai_securities:
			log.info(stock)
			# 股票收盘价列表（元素个数=交易日天数）
			stock_close = prices[stock]['close'].values
			
			# 股票收益率列表(元素个数=交易日天数-1)
			stock_rate = np.diff(stock_close) / stock_close[:-1]

			df_stock = df[df['factor_symbol'].isin([stock])].copy()
			log.info(df_stock)
			
			df_stock = df_stock.iloc[1:, :]

			df_stock['rate'] = stock_rate
			# log.info(df_stock)
			
			# 去极值-标准化
			for index, row in list(df_stock.iteritems())[1:-1]:
				df_stock[index] = standardize(filter_extreme_3sigma(row))
			df_stock = df_stock.fillna(0)

			log.info(df_stock)
	# 第三步,(按有效因子值将个股分为十档，计算每一档的IC、IR、根据IC特征设定方向，根据每组IC均值设定因子权重)
	# 随机选取50只股票来计算IC、IR、根据IC特征设定方向，根据每组IC均值设定因子权重
...
def handle_bar(context, bar_dict):
	...
	if last_date != context.last_date and len(list(context.portfolio.stock_account.positions.keys())) > 0:
		# 如果不是调仓日且有持仓，择时买入
		trade_timing(context, bar_dict)

# 择时买股函数
def trade_timing(context, bar_dict):
	pass
...
def get_needs(context, bar_dict, date, stocks):
	...
	# 第二步,随机选取num只股票来计算IC、IR、根据IC特征设定方向，根据每组IC均值设定因子权重（降低运算数量）
    ...
    for index,jj in enumerate(g.matrix):
        ...
        if g.matrix[index][8] == 1:
            for t in g.record['symbol']:
                if t == sec:
                    ...
                    if (g.record[g.record['symbol'] == sec]['last_buy_price'].empty):
                        ...
                        if current_price > add_price and add_unit < 4:  # 价格上涨超过0.5N并且加仓次数小于4次
                            log.info("加仓")
                            log.info(sec)
                            ...
                            try:
                                if g.record[g.record['symbol'] == sec]['add_time'] == 0:
                                    order(sec, 1.5 * unit)  # 买入2unit的股票
                                    log.info("买入2unit的股票")
                                elif g.record[g.record['symbol'] == sec]['add_time'] == 1:
                                    order(sec, 1.25 * unit)  # 买入1.5unit的股票
            ...
            if (g.matrix[index][1] == True and g.matrix[index][3] == True and g.matrix[index][4] == True) or (
                    g.matrix[index][6] == True and flag_buy == 0):
                g.num += 1
                log.info("购入大盘")
                order_target_value(stock, context.portfolio.available_cash / 40)
                if len(g.record) != 0:
                    g.record = g.record[
                        g.record['symbol'] != stock]  # 清空g.record中sec过期的记录，因为如果之前记录就有000001，现在将这个股票建仓，就要先删除原来为symbol的记录。
        ...
        if g.matrix[index][8] == 0:
                ...
                if t == sec:
                    ...
                    if (g.record[g.record['symbol'] == sec]['last_buy_price'].empty):
                        ...
                        if current_price > add_price and add_unit < 4 and current_price >= (
                                last_price - 2 * atr):  # 价格上涨超过0.5N并且加仓次数小于4次
                            log.info("加仓")
                            log.info(sec)
                            unit = calcUnit(context.portfolio.portfolio_value, atr)  # 计算加仓时应购入的股票数量
                            log.info(unit)
                            try:
                                if g.record[g.record['symbol'] == sec]['add_time'] == 0:
                                    order(sec, 1.5 * unit)  # 买入1unit的股票
                                elif g.record[g.record['symbol'] == sec]['add_time'] == 1:
                                    order(sec, 1.25 * unit)  # 买入1unit的股票
                                elif g.record[g.record['symbol'] == sec]['add_time'] == 2:
                                    order(sec, 1 * unit)  # 买入1unit的股票
                                elif g.record[g.record['symbol'] == sec]['add_time'] == 3:
                                    order(sec, 0.7 * unit)  # 买入1unit的股票
            ...
            if ((g.matrix[index][1] == True and g.matrix[index][3] == True and g.matrix[index][4] == True) or (
                    g.matrix[index][6] == False) and (flag_buy == 0)):
...
def trade_before_new(context,bar_dict):
    ...
    xiaopan = '000905.SH'
    dapan = '000300.SH'
    Indexprice_xiao = get_price(xiaopan, None, time.strftime("%Y%m%d"), '1d', ['close'], True, None, 100, is_panel=1)
    Indexprice_da = get_price(dapan, None, time.strftime("%Y%m%d"), '1d', ['close'], True, None, 100, is_panel=1)
